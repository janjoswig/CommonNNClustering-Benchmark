...
from commonnnbm import helper_timeit
...
def time_unit(bm_unit, repeats=10):
    ...
    if bm_unit.transform_func is not None:
        data = bm_unit.transform_func(
            data, *bm_unit.transform_args, **bm_unit.transform_kwargs
            )
    ...
    for _ in range(repeats):
        timed_func = bm_unit.setup_func(
            data, *bm_unit.setup_args, **bm_unit.setup_kwargs,
            )
...
def profile_unit(bm_unit):
    """Perform a single profiling (e.g. with memory profiler or scalene)
    ...
    timed_func(*bm_unit.timed_args, **bm_unit.timed_kwargs)
...
def parse_standard_arguments():
    ...
    if args.identifier is None:
        args.identifier = (
            f"{args.n_points}_"
            f"{args.n_dim}_"
            f"{args.radius_cutoff}_"
            f"{args.similarity_cutoff}"
        )

    return args


def total_increment_from_memprof_report(report):
    factor_map = {"MiB": 1, "GiB": 1024}
    ...
    for line in report.splitlines():
        if line.startswith("Saving timings report to"):
            report_file = pathlib.Path(line.split(":")[1].strip()).absolute()

        if line.startswith("===="):
            report_began = True
            continue

        if not report_began: continue

        elements = line.split()
        if len(elements) < 6: continue
        try:
            total_mem = float(elements[1])
        except ValueError:
            continue
        factor = factor_map[elements[2]]
        total_mem *= factor
        if total_mem < min_mem: min_mem = total_mem
        if total_mem > max_mem: max_mem = total_mem

    total_mem = max_mem - min_mem
    print("Saving timings report to: ", report_file)
    with open(report_file, 'a+') as ofile:
        ofile.write(f"{total_mem:.2f}\n")
